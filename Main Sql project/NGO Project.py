...
def new_questions2(password):
    if password=='bhavans':
        ...
        for i in range(int(input('NUMBER OF QUESTIONS : '))):
            ...
            an=input('ANSWER : ')
            que=q+'\n\n\n'+o1+'\n\n\n'+o2+'\n\n\n'+o3+'\n\n\n'+o4
            query='insert into questions2 values("'+que+'",'+l+',"'+an+'")'
            c.execute(query)
            a.commit()
    else:
...
a=my.connect(host='localhost',user='root',passwd='Aryan@123',database='csproject_bhavans')
c=a.cursor()
c.execute('select * from questions2')
for i in c:
    questions2.append(i)
questions2_attempted=[]
random_elist=[]
random_mlist=[]
random_hlist=[]
esy=[]
...
def easy(number):
    def speaking():
        ...
        speak(a)
        newvoicerate = 10
        speaking.setproperty('rate',10 )

    def submite():
        ans=easyv.get().upper()
        questions2_attempted[-1]+=[ans]
        if ans==que[1]:
    ...
    questions2_attempted.append(esy[index])
...
def medium(number):
    ...
    questions2_attempted.append(med[index])
    Label(win,text='QUESTION '+str(number)+".",font=("Arial",30)).pack(pady=20)
    Label(win,text=que[0],font=("Arial",20)).pack(pady=20)
    mediumv=StringVar()
...
def hard(number):
    ...
    questions2_attempted.append(hrd[index])
    Label(win,text='QUESTION '+str(number)+".",font=("Arial",30)).pack(pady=20)
    Label(win,text=que[0],font=("Arial",20)).pack(pady=20)
    hardv=StringVar()
...
if check[-1]:
    ...
    def analyse():
        ...
        if lst_helper[-1]:
            toquit=[1]
            j=0
            while j<len(questions2_attempted) and toquit[-1]:
                i=questions2_attempted[j]
                win=Tk()
